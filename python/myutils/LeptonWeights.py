            ...
            if self.channel == 'Zll' or len(self.channel) < 1:
                ...
                for j, name in jsons.items():
                    ...
                    if not j.find('trk_SF_Run') != -1 and not j.find('EfficienciesAndSF_dZ_numH') != -1:
                        if 'abseta' in  name[1]:
                            self.weight.append(lepCorr.get_2D(eta=abs(tree.vLeptons_new_eta[0]), pt=tree.vLeptons_new_pt[0]))
                            self.weight.append(lepCorr.get_2D(eta=abs(tree.vLeptons_new_eta[1]), pt=tree.vLeptons_new_pt[1]))
                        else:
                            self.weight.append(lepCorr.get_2D(eta=tree.vLeptons_new_eta[0], pt=tree.vLeptons_new_pt[0]))
                            self.weight.append(lepCorr.get_2D(eta=tree.vLeptons_new_eta[1], pt=tree.vLeptons_new_pt[1]))
                    elif not j.find('trk_SF_Run') != -1 and j.find('EfficienciesAndSF_dZ_numH') != -1:
                        #???????
                        self.weight.append(lepCorr.get_2D(tree.vLeptons_new_eta[0], tree.vLeptons_new_eta[1]))
                        self.weight.append(lepCorr.get_2D(tree.vLeptons_new_eta[1], tree.vLeptons_new_eta[0]))
                    #1-D binned SF
                    else:
                        self.weight.append(lepCorr.get_1D(tree.vLeptons_new_eta[0]))
                        self.weight.append(lepCorr.get_1D(tree.vLeptons_new_eta[1]))
                    ...
                    if tree.Vtype_new == 0:
                        ...
                        if j.find('EIDISO_ZH_out') != -1:
                            ...
                            self.computeSF_region(self.branchBuffers['weight_SF_LooseIDnISO_B'], self.branchBuffers['weight_SF_LooseIDnISO_E'], tree.vLeptons_new_eta[0], tree.vLeptons_new_eta[1], 1.566)
                        #TRK
                ...
                if tree.Vtype_new != 4:
                    ...
                    for j, name in jsons.items():
                        ...
                        if not j.find('trk_SF_Run') != -1:
                            if 'abseta' in  name[1]:
                                self.weight.append(lepCorr.get_2D(abs(tree.vLeptons_new_eta[0]), tree.vLeptons_new_pt[0]))
                            else:
                                self.weight.append(lepCorr.get_2D(tree.vLeptons_new_eta[0], tree.vLeptons_new_pt[0]))
                        #1-D binned SF
                        else:
                            self.weight.append(lepCorr.get_1D(tree.vLeptons_new_eta[0]))
